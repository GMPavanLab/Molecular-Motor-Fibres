...
for i in $(seq 0 15); do
    ...
    gmx grompp -f mdp/emin3.mdp -c ../tmp/em_${NAME}.gro -p $TOPOLOGY_PATH -o ../tmp/em_${NAME}.tpr -maxwarn 1
    gmx mdrun -deffnm ../tmp/em_${NAME} -nt 16 -gpu_id $GPU_ID
    echo $(tail -n 1 ../tmp/em_${NAME}.gro)
    cp ../tmp/em_${NAME}.gro ../tmp/em_${NAME}_$i.gro
done

for i in $(seq 16 $((16 + N_SCALES - 1)) ); do
    echo "Iteration $i"
    ...
    gmx grompp -f mdp/emin3.mdp -c ../tmp/em_whole_${NAME}.gro -p $TOPOLOGY_PATH -o ../tmp/em_${NAME}.tpr -maxwarn 1 > /dev/null 2>&1
    gmx mdrun -deffnm ../tmp/em_${NAME} -nt 16 -gpu_id $GPU_ID
    echo $(tail -n 1 ../tmp/em_${NAME}.gro)
    cp ../tmp/em_${NAME}.gro ../tmp/em_${NAME}_$i.gro
done
...
EOF
gmx grompp -f mdp/npt_short.mdp -c ../tmp/em_${NAME}.gro -p $TOPOLOGY_PATH -o ../tmp/npt_short_${NAME}.tpr -maxwarn 1 > /dev/null 2>&1
gmx mdrun -deffnm ../tmp/npt_short_${NAME} -nt 16 -gpu_id $GPU_ID

mkdir -p ../output/${NAME}
cp ../tmp/npt_short_${NAME}.gro ../output/${NAME}/${NAME}_dense.gro
cp $TOPOLOGY_PATH ../output/${NAME}/${NAME}.top
