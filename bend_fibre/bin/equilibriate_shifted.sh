...
#gmx mdrun -deffnm ../run/eq1_vacuum/eq -v -gpu_id $GPU_ID -nt $NCORES
...
gmx grompp -f ../bin/mdp/em.mdp -c ../run/em_vacuum/em.gro -p $TOPOLOGY_PATH -o ../tmp/tmp.tpr -maxwarn 1
echo 0 | gmx trjconv -f ../run/em_vacuum/em.gro -s ../tmp/tmp.tpr -o ../tmp/fibre_whole.gro -pbc whole
gmx editconf -f ../tmp/fibre_whole.gro -c -o ../tmp/box.gro -box 20 20 $(echo "$zlen+1" | bc)

#Solvate system
gmx solvate -cp ../tmp/box.gro  -cs $solvent_box \
   -o ../tmp/box_sol.gro -p ${TOPOLOGY_PATH}
python remove_water_in_fibre_shifted.py ../tmp/box_sol.gro $TOPOLOGY_PATH ../tmp/box_clean.pdb -r 25
...
INDEX=../run/${OUTPUT_NAME}.ndx
gmx make_ndx -f ../tmp/box_clean.pdb -o ${INDEX} <<EOF
2 | 3 | 4 | 5
6 | 7
name 8 MOTOR
name 9 SOL_ION
...
#Equilibriate water
gmx grompp -p ${TOPOLOGY_PATH} -c ../tmp/box_clean.pdb -f mdp/em_solvant.mdp  \
 -r ../tmp/box_clean.pdb -o ../run/em_solvant/em_W.tpr  -maxwarn 1
gmx mdrun -v -deffnm ../run/em_solvant/em_W -gpu_id $GPU_ID -nt $NCORES

gmx grompp -p ${TOPOLOGY_PATH} -c ../run/em_solvant/em_W.gro -f mdp/eq_solvant2.mdp  \
   -r ../tmp/box_clean.pdb -o ../run/eq_solvant/eq_W.tpr -maxwarn 1 -n $INDEX
