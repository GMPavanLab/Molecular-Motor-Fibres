...
gmx genconf -f $FIBRE -o ../tmp/double_box.gro -nbox 1 1 4
sed -n '/\[ molecules \]/,$p' $TOPOLOGY | sed 1,2d >> $TOPOLOGY #Duplicate molecule numbers in top file
sed -n '/\[ molecules \]/,$p' $TOPOLOGY | sed 1,2d >> $TOPOLOGY
...
structure $FIBRE
  number 1
  center
  fixed $(echo "scale=2; ($DIST / 2 )" | bc) -$(echo "scale=2; $DIST*sqrt(1- 1/4)" | bc) 0. 0. 0. 0.
end structure 
structure $FIBRE
  number 1
  center
...
gmx editconf -f ../tmp/packmol.pdb -o ../tmp/distbox.gro -d 0.1 -c
gmx editconf -f ../tmp/distbox.gro -o $OUTPUT_FILE_NAME -box 20 20 $boxz
