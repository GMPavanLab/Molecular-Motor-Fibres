...
gmx grompp -p ${TOPOLOGY_PATH} -c ../run/em_vacuum/em.gro -n $INDEX -f mdp/eq1_vacuum.mdp  -o ../run/eq1_vacuum/eq.tpr -maxwarn 1
gmx mdrun -deffnm ../run/eq1_vacuum/eq -gpu_id $GPU_ID -nt $NCORES

for i in {2..5}; do
    mkdir -p ../run/eq${i}_vacuum
    gmx grompp -p ${TOPOLOGY_PATH} -c ../run/eq$(( $i - 1 ))_vacuum/eq.gro -f mdp/eq${i}_vacuum.mdp \
     -n $INDEX -o ../run/eq${i}_vacuum/eq.tpr  -maxwarn 1
    gmx mdrun -v -deffnm ../run/eq${i}_vacuum/eq -gpu_id $GPU_ID -nt $NCORES -nsteps 10000
done'

#Solvate system
gmx solvate -cp ../run/eq5_vacuum/eq.gro  -cs ../structure_files/box_CG_W_eq.gro \
...
gmx grompp -p ${TOPOLOGY_PATH} -c ../tmp/box_clean.gro -f mdp/em_solvant.mdp  \
 -r ../tmp/box_clean.gro -o ../run/em_solvant/em_W.tpr
...
gmx grompp -p ${TOPOLOGY_PATH} -c ../run/em_solvant/em_W.gro -f mdp/eq_solvant2.mdp  \
   -r ../tmp/box_clean.gro -o ../run/eq_solvant/eq_W.tpr -n $INDEX
