...
def radius(universe, resnames=None):
    ...
    for i, ts in enumerate(universe.trajectory):
        ...
        margin_x, margin_y = box[0] * 0.075, box[1] * 0.075
...
def torsion(universe, angle=None, angle_name=None):
    ...
    for i, ts in enumerate(universe.trajectory):
        ...
        for mol in molecules:
            ...
            first_abs_idx = int(mol.atoms.indices[0])  # absolute atom index in the Universe
            mol_key = f"{int(mol_resid)}_{first_abs_idx}"
...
def main():
    ...
    universe = mda.Universe(args.structure, args.trajectory, bonds='all', to_guess=['bonds', 'masses'], topology_format='PDB', vdwradii={'Na': 0.0})
